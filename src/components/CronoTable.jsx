...
export default function CronoTable({
  ...
  return (
    <div className="crono" style={{ "--accent": accent }}>
      <div className="crono__header">
        {isDoble
          ? "Cuota doble — Jul y Dic se paga 2×"
          : "Cuota simple — mismo monto cada mes"}
      </div>
              ...
              return (
                <tr key={r.mes} className={rowClass}>
                  <td className="crono__cell crono__cell--n">
                    {r.mes}
                    {r.esDoble && (
                      <span className="crono__doble-mark">×2</span>
                    )}
                  </td>
                  <td className="crono__cell crono__cell--fecha">
                    {formatDate(r.fecha)}
                  </td>
                  <td className="crono__cell crono__cell--dias">{r.dias}</td>
                  <td className="crono__cell crono__cell--amort">
                    {fmt(r.amortizacion)}
                  </td>
                  <td className="crono__cell crono__cell--interes">
                    {fmt(r.interes)}
                  </td>
                  <td className="crono__cell crono__cell--seg-desg">
                    {fmt(r.seguroDesgravamen)}
                  </td>
                  <td className="crono__cell crono__cell--seg-inm">
                    {fmt(r.seguroInmueble)}
                  </td>
                  <td className="crono__cell crono__cell--cuota">
                    {fmt(r.cuotaFija)}
                  </td>
                  <td className="crono__cell crono__cell--saldo">
                    {fmt(r.saldo)}
                  </td>
                </tr>
              );
      ...
      {visibleRows < crono.length && (
        <button
          onClick={() =>
            setVisibleRows((v) => Math.min(v + 24, crono.length))
          }
