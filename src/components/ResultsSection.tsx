...
import type { CalcResult, CronoMode } from "@/lib/types";
...
type ResultsSectionProps = {
  ...
  guardar: () => void;
  cronoMode: CronoMode;
  setCronoMode: Dispatch<SetStateAction<CronoMode>>;
  visibleRows: number;
...
export default function ResultsSection({
  ...
  const cards: Array<{
    label: string;
    cuota: number;
    data: { totalIntereses: number; totalSeguros: number; totalPagado: number };
    accent: string;
    accentBg: string;
    dobleVal?: number;
  }> = [
  ...
  const firstRow = result.simple.cronograma[0];
  const stats = [
    { label: "TEM ref.", val: (result.tem * 100).toFixed(4) + "%" },
    { label: "Tasa diaria", val: (result.tasaDiaria * 100).toFixed(6) + "%" },
    { label: "1er mes", val: (firstRow ? firstRow.dias : 0) + " días" },
  ];
  ...
  return (
    <div className="results">
      ...
      <div className="summary-grid">
        {cards.map(({ label, cuota, data, accent, accentBg, dobleVal }) => {
          const cardStyle: CSSVars = { "--accent": accent, "--accent-bg": accentBg };
          return (
            <div key={label} className="summary-card" style={cardStyle}>
              <div className="summary-card__label">{label}</div>
              <div className="summary-card__amount">S/ {fmt(cuota)}</div>
              {dobleVal && <div className="summary-card__sub">Jul/Dic: S/ {fmt(dobleVal)}</div>}
              <div className="summary-card__stats">
                <div className="summary-card__stat-row">
                  <span>Total intereses</span>
                  <span className="text-red">S/ {fmtInt(data.totalIntereses)}</span>
                </div>
                <div className="summary-card__stat-row">
                  <span>Total seguros</span>
                  <span className="text-amber">S/ {fmtInt(data.totalSeguros)}</span>
                </div>
      ...
      <div className="crono-toggle">
        {TOGGLES.map(({ key, label, color }) => {
          const btnStyle: CSSVars = { "--accent": color };
          return (
            <button
              key={key}
              onClick={() => {
